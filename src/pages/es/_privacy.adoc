...
Una cadena de caracteres anonimizada creada a partir de tu dirección de correo electrónico (también conocida como hash) puede ser proporcionada al servicio de Gravatar para ver si la estás utilizando. Puedes encontrar la política de privacidad del servicio de Gravatar aquí: link:https://automattic.com/privacy/[]. Después de aprobar tu comentario, tu imagen de perfil será visible al público en el contexto de tu comentario.
...
Si dejas un comentario, el comentario y sus metadatos se conservarán indefinidamente. Esto nos permite detectar y aprobar automáticamente todos los comentarios de seguimiento en lugar de mantenerlos en una cola de moderación.

Para usuarios que se registran en nuestro sitio web (si los hay), también almacenamos la información personal que proporcionan en su perfil de usuario. Todos los usuarios pueden ver, editar o eliminar sus datos personales en cualquier momento (excepto que no pueden cambiar su nombre de usuario). Los administradores del sitio web también pueden ver y editar esta información.

=== Qué derechos tiene sobre sus datos

Si tiene una cuenta en este sitio web o ha dejado comentarios, puede solicitar que reciba un archivo exportado de los datos personales que tenemos sobre usted, incluidos los datos que nos ha proporcionado. También puede solicitar que eliminemos cualquier dato personal que tengamos sobre usted. Esto no se aplica a los datos que estamos obligados a retener por razones administrativas, legales o de seguridad.
...
Utilizamos principios de diseño de seguridad de última generación y cumplimos estrictamente con el Reglamento General de Protección de Datos Global (RGPD). Para más información, consulte también la política de privacidad de MEtropoliplan, accesible desde el menú principal.

Todos los datos se cifran durante el transporte y en reposo (p. ej., SSL, cifrado de base de datos, etc.)

=== Qué procedimientos de incumplimiento de datos hemos implementado

En caso de una incidencia de datos, todos los administradores de inquilinos serán informados para comunicar los siguientes pasos necesarios al usuario.

=== De qué terceros recibimos datos

No recibimos datos adicionales de terceros.

=== Qué toma de decisiones automatizada o perfilado hacemos con datos de usuario

A nivel del sitio web, no hay toma de decisiones automatizada ni perfilado. Dentro de MEtropoliPlan, tenemos un robot de mapeo automatizado.

=== Requisitos de divulgación de la industria

Esto no se aplica a este sitio web ni al software MEtropoliplan.
